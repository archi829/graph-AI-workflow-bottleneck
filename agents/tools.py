...
def make_local_knowledge_tool(fail_probability: float = 0.0):
    ...
    def local_knowledge(topic: str) -> str:
        """Look up known facts about a product, destination, or bug pattern
        from the local knowledge base. Use this before falling back to web_search.

        If your query mentions multiple products/topics together, this
        returns facts for ALL of them rather than just one -- but you'll
        get more reliable results querying one product/topic per call.
        """
        ...
        matches = [v for k, v in _KNOWLEDGE_BASE.items() if k in key or key in k]
        if matches:
            return "\n".join(matches)
        return f"No local knowledge entry for '{topic}'. Try web_search instead, or query one product/topic at a time."
